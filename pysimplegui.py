...
def show_category(category):
        ...
        if category == 'All Apps':
            category_apps = os.popen(f'{DIRECTORY}/api list_apps cpu_installable').read().split('\n')          

        elif category != 'Installed': 
            category_apps = os.popen(f'cat  {DIRECTORY}/etc/categories  {DIRECTORY}/data/category-overrides | grep ' + category + ' | sed "s/|.*//g"').read().split('\n')               
        
        else: # Installed
            category_apps = os.popen(f'{DIRECTORY}/api list_apps installed').read().split('\n')
...
def load_app_info(app):
    ...
    status_text = os.popen(f'{DIRECTORY}/api app_status "' + app + '"').read().rstrip('\n')
    if status_text == '':
    ...
    if status_text == 'corrupted':
        ...
        window["-INSTALL OR UNINSTALL-"].update(visible=True, image_filename=f'{DIRECTORY}/icons/install.png')
        window["-UNINSTALL-"].update(visible=True, image_filename=f'{DIRECTORY}/icons/uninstall.png')
    elif status_text == 'uninstalled':
        window["-INSTALL OR UNINSTALL-"].update(visible=True, image_filename=f'{DIRECTORY}/icons/install.png')
    elif status_text == 'installed':
        window["-INSTALL OR UNINSTALL-"].update(visible=True, image_filename=f'{DIRECTORY}/icons/uninstall.png')
    current_app = app
...
search_column = [
    ...
    [
        sg.Button(key='-MENU BACK-', image_filename=f'{DIRECTORY}/icons/back.png', button_text=' ', visible=False)
    ],
]
    ...
    if event == "-SEARCH-":
        ...
        try:
            if search_query != '': # If search query is not empty
                output = os.popen(f'{DIRECTORY}/api list_apps cpu_installable | {DIRECTORY}/api list_intersect "$({DIRECTORY}/api list_apps visible)" | grep -i "' + \
                ...
                window['-APP LIST-'].update(app_list_data) # Update to the app_list_data generated just now
                window['-MENU BACK-'].update(visible=True) # Make back button visible so users can return to category list
        ...
        window['-DESCRIPTION-'].Widget.bind("<1>", window['-DESCRIPTION-'].Widget.focus_set())
        ...
        if current_app != '':
            if current_app not in app_categories:
                app = current_app
                credits_text = os.popen(f'cat "{DIRECTORY}/apps/' + app + '/credits" 2>/dev/null').read().rstrip('\n')
                if credits_text != '':
                ...
                if os.popen(f'{DIRECTORY}/api app_type ' + app).read().strip('\n') != 'package':
                    for file in ['install', 'install-32', 'install-64', 'uninstall']:
                        if os.path.exists(f'{DIRECTORY}/apps/' + app + '/' + file):
                            os.popen(f'{DIRECTORY}/api text_editor "{DIRECTORY}/apps/' + app + '/' + file + '"')
                else:
                    sg.Window('No scripts found', [[sg.T(app + ' is a package app.\n')], [sg.OK(s=10)]], size=(500, 100)).read(close=True)
        ...
        if current_app != '':
            app = current_app
            print(app)
            error_file = os.popen(f'ls "{DIRECTORY}/logs"/* -t | grep "fail-' + app + '" | head -n1').read()
